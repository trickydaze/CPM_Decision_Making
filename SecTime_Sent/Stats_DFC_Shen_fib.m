...
Grp_con = ones(length(Cards_data),1); Grp_Fibro_Hyp = 2*(ones(length(Food_data),1));
Grp = [Grp_con; Grp_Fibro_Hyp]; 
Grp_meta = [Cards_meta',Food_meta'];
...
%%
    figure(); plot(mean(Discount_int,2), 'b-o', 'LineWidth',3,'MarkerSize',12); hold on; plot(mean(Food_int,2), 'r--*', 'LineWidth',3,'MarkerSize',12)
    title('Whole Brain Integration'); ylabel('Metastability'); xlabel('No of Subjects')
    set(gca,'FontWeight','bold','FontName', 'Arial','fontsize',18)
    legend('Discount','Food')
    %%
...
set(gca,'FontSize', 20, 'FontName','Calibri', 'FontWeight', 'bold')
%%
    figure(); plot(mean(Discount_seg,2), 'b-o', 'LineWidth',3,'MarkerSize',12); hold on; plot(mean(Food_seg,2), 'r--*', 'LineWidth',3,'MarkerSize',12); hold on; plot(mean(Cards_seg,2),':s', 'LineWidth',3,'MarkerSize',12)
    title('Whole Brain Segregation'); ylabel('Segregation'); xlabel('No of Subjects')
    set(gca,'FontWeight','bold','FontName', 'Arial','fontsize',18)
    legend('Discount','Food','Cards')
