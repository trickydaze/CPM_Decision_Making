function varargout=notBoxPlot(y,x,jitter,style,faceflag)
...
if isvector(y) && isvector(x) && length(x)>1
    ...
    for ii=1:length(u)
        f=x==u(ii);
        h(ii,:)=notBoxPlot(y(f),u(ii),jitter,style,faceflag(f));
    end
...
for ii=1:length(uX)
    f=b==ii;
    h=[h,myPlotter(x(f),y(:,f),faceflag)];
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function h=myPlotter(X,Y,FLAG)

%  SEM=SEM_calc(Y); %Supplied external function
 SD=nanstd(Y);  %Requires the stats toolbox
 SEM=1.960*(SD/sqrt(sum(~isnan(Y))));  %SEM=SD/sqrt(sum(~isnan(Y)));  CI: 90%=1.645; 95%=1.960; 99%=2.576
...
%      end
     ...
     h(k,1).data=plot(thisX(FLAG == 1)+J(FLAG == 1), thisY(FLAG == 1), 'o', 'color', C,...
                   'markerfacecolor', C+(1-C)*0.65);
               
     h(k,2).data=plot(thisX(FLAG ~= 1)+J(FLAG ~= 1), thisY(FLAG ~= 1), 'o', 'color', C,...
